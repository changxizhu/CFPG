...
class ReplayBuffer(EpisodeBatch):
    ...
    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu", out_device=None, seed=None):
        ...
        # set random state for replay buffer
        self.seed = seed
        if self.seed:
    ...
    def insert_episode_batch(self, ep_batch):
        # print("Insert batch:", ep_batch.batch_size, self.buffer_index)
    ...
    def sample(self, batch_size):
        ...
        if self.episodes_in_buffer == batch_size:
            ...
            # Uniform sampling only atm: this will also change the random seed
            if self.rs:
                ep_ids = self.rs.choice(self.episodes_in_buffer, batch_size, replace=False).tolist()
            else:
