    ...
    def run(self, test_mode=False, **kwargs):
        ...
        while True:
            ...
            actions = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated,
                                              test_mode=test_mode)
            # print("actions before in buffer:", actions.unsqueeze(1))
            ...
            # if test_mode:
            #     # print("Obs:", np.mean(self.batch["obs"][:, self.t], axis=-1))
            #     print("CPU norm:", np.linalg.norm(cpu_actions[0]), "CPU actions:", cpu_actions[0])
            #     print("Test:", test_mode, " Average rewards per step:", env_rewards[0])
