...
class ParallelAttentionRunner:
    ...
    def __init__(self, args, logger):
        ...
        if self.args.env == "sc2":
            ...
            self.ps = []
            for env_id, worker_conn in enumerate(self.worker_conns):
                # self.args.env_args['seed'] = np.random.randint(1e6)
                self.args.env_args['env_id'] = env_id
                print("Random env:", self.args.env_args)
                process = Process(target=env_worker, args=(worker_conn, CloudpickleWrapper(partial(env_fn, env_args=self.args.env_args.copy(), args=self.args))))
        ...
        print("Load env info:", self.env_info)
    ...
    def run(self, test_mode=False, **kwargs):
        ...
        if not test_mode and not self.args.parallel_training:
            parent_conns = self.parent_conns[0:1]
            # parent_conns = self.parent_conns
        else:
            # print("Using parallel programming!!!!!!!!!!!!!!!!!")
            parent_conns = self.parent_conns
        ...
        while True:
            ...
            if getattr(self.args, "allow_messages", False) == True:
                messages =  self.mac.select_messages(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated,
                                                   test_mode=test_mode)  # not explore but can be changed later
                
                # 1. store norm and mean of messages
                ...
                self.batch.update(messages_data, bs=envs_not_terminated, ts=self.t, mark_filled=False)
            ...
            # print("Parallel learner:", self.batch.batch_size, self.t, envs_not_terminated, " Test mode:", test_mode)
            actions = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated,
                                              test_mode=test_mode)
            # print("Actions shape:", actions.unsqueeze(1).shape)
            ...
            # render environment
            if self.args.render:
                parent_conns[0].send(("render", None))
                parent_conns[0].recv()

            # print("num of envs:", len(parent_conns))
            ...
            for idx, parent_conn in enumerate(parent_conns):
                if not terminated[idx]:
                    ...
                    env_rewards.append(data["reward"])
            ...
            if (not test_mode) and (getattr(self.args, "runner_scope", "episodic") == "transition"):
                ...
                # print("Insert batch:", self.batch.batch_size, self.batch[0, self.t - 1:self.t + 1].max_seq_length)
                buffer.insert_episode_batch(self.batch[0, self.t - 1:self.t + 1])

                if self.args.use_dynamics and (self.t_env + self.t - self.last_learn_T_dy) / self.args.learn_interval >= 1.0:
                    # execute learning steps (if enabled)
                    # print("Buffer:", buffer.can_sample(self.args.batch_size), buffer.episodes_in_buffer, self.args.batch_size)
                    if buffer.can_sample(self.args.batch_size) and (buffer.episodes_in_buffer > getattr(self.args, "buffer_warmup", 0)):
                        """
                        train dynamics model
                        """
                        episode_sample_dy = buffer.sample(self.args.batch_size)

                        # Truncate batch to only filled timesteps
                        max_ep_t = episode_sample_dy.max_t_filled()
                        episode_sample_dy = episode_sample_dy[:, :max_ep_t]

                        if episode_sample_dy.max_seq_length != 0:    # max_t_filled may truncate all samples
                            if episode_sample_dy.device != self.args.device:
                                episode_sample_dy.to(self.args.device)

                            if self.args.verbose:
                                print("Learning dynamics now for {} steps...".format(getattr(self.args, "n_train", 1)))
                            for _ in range(getattr(self.args, "n_train", 1)):
                                learner.train_dynamics(episode_sample_dy, self.t_env, episode)
                            self.last_learn_T_dy = self.t_env + self.t
                        else:
                            print("All samples have been truncated:", max_ep_t)  
                ...
                if (self.t_env + self.t - self.last_learn_T) / self.args.learn_interval >= 1.0:
                    ...
                    if buffer.can_sample(self.args.batch_size) and (buffer.episodes_in_buffer > getattr(self.args, "buffer_warmup", 0)):
                        """
                        train critics, actors and communication model
                        """
                        ...
                        if episode_sample.max_seq_length != 0:    # max_t_filled may truncate all samples
        ...
        if not test_mode:
            ...
            if self.args.record_att and self.episodes%100 == 0 and attention_weights != None:  # record attention weights every 50 episodes
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        # print("Remote command and data:", cmd, data)
        if cmd == "step":
            actions = data
