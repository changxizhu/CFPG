...
class G2ANetCritic(nn.Module):
    def __init__(self, scheme, args):
        super(G2ANetCritic, self).__init__()
        self.args = args
        ...
        # Encoding
        self.encoding = nn.Linear(self.input_shape, args.critic_rnn_hidden_dim)  # 对所有agent的obs解码
        self.h = nn.GRUCell(args.critic_rnn_hidden_dim, args.critic_rnn_hidden_dim)  # 每个agent根据自己的obs编码得到hidden_state，用于记忆之前的obs

        # Hard
        # GRU输入[[h_i,h_1],[h_i,h_2],...[h_i,h_n]]与[0,...,0]，输出[[h_1],[h_2],...,[h_n]]与[h_n]， h_j表示了agent j与agent i的关系
        # 输入的iputs维度为(n_agents - 1, batch_size * n_agents, critic_rnn_hidden_dim * 2)，
        # 即对于batch_size条数据，输入每个agent与其他n_agents - 1个agents的hidden_state的连接
        self.hard_bi_GRU = nn.GRU(args.critic_rnn_hidden_dim * 2, args.critic_rnn_hidden_dim, bidirectional=True)
        # 对h_j进行分析，得到agent j对于agent i的权重，输出两维，经过gumble_softmax后取其中一维即可，如果是0则不考虑agent j，如果是1则考虑
        self.hard_encoding = nn.Linear(args.critic_rnn_hidden_dim * 2, 2)  # 乘2因为是双向GRU，hidden_state维度为2 * hidden_dim

        # Soft
        self.q = nn.Linear(args.critic_rnn_hidden_dim, args.attention_dim, bias=False)
        self.k = nn.Linear(args.critic_rnn_hidden_dim, args.attention_dim, bias=False)
        self.v = nn.Linear(args.critic_rnn_hidden_dim, args.attention_dim)

        # Decoding 输入自己的h_i与x_i，输出自己动作的概率分布
        self.decoding = nn.Linear(args.critic_rnn_hidden_dim + args.attention_dim, 1)  # out put the Q-value for the joint action


    def init_hidden(self, batch_size):
        # make hidden states on same device as model
        self.hidden_states = self.encoding.weight.new(1, self.args.critic_rnn_hidden_dim).zero_().unsqueeze(0).expand(batch_size, self.n_agents, -1)

    def forward(self, inputs, actions, hidden_state):
        if actions is not None:
            inputs = torch.cat([inputs.view(-1, self.input_shape - self.n_actions),
                             actions.contiguous().view(-1, self.n_actions)], dim=-1)
        size = inputs.shape[0]  # batch_size * n_agents
        # 先对obs编码
        obs_encoding = f.relu(self.encoding(inputs))
        h_in = hidden_state.reshape(-1, self.args.critic_rnn_hidden_dim)

        # 经过自己的GRU得到h
        h_out = self.h(obs_encoding, h_in)  # (batch_size * n_agents, args.critic_rnn_hidden_dim)

        # Hard Attention，GRU和GRUCell不同，输入的维度是(序列长度,batch_size, dim)
        if self.args.hard:
            # Hard Attention前的准备
            h = h_out.reshape(-1, self.n_agents, self.args.critic_rnn_hidden_dim)  # 把h转化出n_agents维度，(batch_size, n_agents, critic_rnn_hidden_dim)
            input_hard = []
            for i in range(self.n_agents):
                h_i = h[:, i]  # (batch_size, critic_rnn_hidden_dim)
                h_hard_i = []
                for j in range(self.args.n_agents):  # 对于agent i，把自己的h_i与其他agent的h分别拼接
                    if j != i:
                        h_hard_i.append(torch.cat([h_i, h[:, j]], dim=-1))
                # j 循环结束之后，h_hard_i是一个list里面装着n_agents - 1个维度为(batch_size, critic_rnn_hidden_dim * 2)的tensor
                h_hard_i = torch.stack(h_hard_i, dim=0)
                input_hard.append(h_hard_i)
            # i循环结束之后，input_hard是一个list里面装着n_agents个维度为(n_agents - 1, batch_size, critic_rnn_hidden_dim * 2)的tensor
            input_hard = torch.stack(input_hard, dim=-2)
            # 最终得到维度(n_agents - 1, batch_size * n_agents, critic_rnn_hidden_dim * 2)，可以输入了
            input_hard = input_hard.view(self.args.n_agents - 1, -1, self.args.critic_rnn_hidden_dim * 2)

            h_hard = torch.zeros((2 * 1, size, self.args.critic_rnn_hidden_dim))  # 因为是双向GRU，每个GRU只有一层，所以第一维是2 * 1
            if self.args.use_cuda:
                h_hard = h_hard.cuda()
            h_hard, _ = self.hard_bi_GRU(input_hard, h_hard)  # (n_agents - 1,batch_size * n_agents,critic_rnn_hidden_dim * 2)
            h_hard = h_hard.permute(1, 0, 2)  # (batch_size * n_agents, n_agents - 1, critic_rnn_hidden_dim * 2)
            h_hard = h_hard.reshape(-1, self.args.critic_rnn_hidden_dim * 2)  # (batch_size * n_agents * (n_agents - 1), critic_rnn_hidden_dim * 2)
