...
class CommActorLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.comm_params = list(mac.comm_parameters())   # parameters of communication
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # print("batch.max_seq_length:", batch.max_seq_length, actions.shape)
        ...
        # if len(q_taken) == 0:
        #     print("Wrong q_taken:", batch.max_seq_length)
        ...
        for t in range(batch.max_seq_length):
            ...
            target_msgs.append(msg_target_outs)
        ...
        for t in range(batch.max_seq_length):
            # use messages in target mac
            # print("Target messages:", target_msgs.shape, target_msgs[:, t:t+1].shape)
        ...
        actions_norm = []
        for t in range(batch.max_seq_length-1):
            # use sampled messages, evaluate at this point
            msg_input = self._build_msg_inputs(batch, t=t)
            sampled_actions = actions[:, t:t + 1]
            agent_outs = self.mac.forward(batch, t=t, messages=msg_input.detach(), select_actions=True, use_direct=True)["actions"].view(batch.batch_size,
                                                                                           self.n_agents,
                                                                                           self.n_actions)
            ...
            if self.mixer is not None:
                ...
                # after_q = self.mixer(after_q.view(batch.batch_size, -1, 1), batch["state"][:, t:t+1])
            mac_out.append(agent_outs)
            chosen_action_qvals.append(q)
            # after_q_values.append(after_q)
        mac_out = th.stack(mac_out, dim=1)
        chosen_action_qvals = th.stack(chosen_action_qvals, dim=1)
        # after_q_values = th.stack(after_q_values, dim=1)
        pi = mac_out
        ...
        # calculate loss by using sampled actions and messages
        # self.critic.init_hidden(batch.batch_size)
        # after_q_values = []
        # for t in range(batch.max_seq_length-1):
        #     # use sampled messages, evaluate at this point
        #     msg_input = self._build_msg_inputs(batch, t=t)
        #     after_q, self.critic.hidden_states = self.critic(self._build_inputs(batch, t=t), msg_input.detach(), actions[:, t:t + 1].detach(),
        #                                                self.critic.hidden_states)
        #     if self.mixer is not None:
        #         after_q = self.mixer(after_q.view(batch.batch_size, -1, 1), batch["state"][:, t:t+1])
        #     after_q_values.append(after_q)
        ...
        for t in range(batch.max_seq_length-1):
            # sampled messages
            sampled_msg_input = self._build_msg_inputs(batch, t=t)
            sampled_actions = actions[:, t:t + 1]
            ...
            # build messages input for both critic and actor
            msg_input = self._build_msg_inputs(batch, t=t, messages=msgs_outs)
            # use current messages to evaluate at this point
            agent_outs = self.mac.forward(batch, t=t, messages=msg_input, select_actions=True, use_direct=True)["actions"].view(batch.batch_size,
                                                                                        self.n_agents,
                                                                                        self.n_actions)
            # calculate 2-norm for sampled messages and current messages
            # print("Messages norm:", pdist(sampled_msg_input, msg_input).mean())
            messages_norm.append(pdist(sampled_msg_input, msg_input).mean().item())
            actions_comm_norm.append(pdist(sampled_actions, agent_outs).mean().item())
            ...
            if self.mixer is not None:
                q = self.mixer(q.view(batch.batch_size, -1, 1), batch["state"][:, t:t+1])
            comm_mac_out.append(msgs_outs)   # regularize the communication function
            mac_out.append(agent_outs)
        ...
        # Compute the actor loss
        pg_comm_loss = -comm_chosen_action_qvals.mean() + (comm_pi**2).mean() * 1e-3 
        # + (pi**2).mean() * 1e-3
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            # self.logger.log_stat("loss_step/after_q_values", after_q_values.sum().item() / mask_elems, t_env)
            ...
            self.logger.log_stat("grad_step/messages_dist", np.mean(messages_norm), t_env)
