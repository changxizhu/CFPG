    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length - 1):
            ...
            for idx in range(self.n_agents):
                ...
                # print("Tem join grad:", idx, tem_joint_act.requires_grad)
                tem_joint_act[:, idx] = agent_outs[:, idx]
                # print("Tem join grad:", idx, tem_joint_act.requires_grad)
                # print("Tem join grad:", idx, tem_joint_act[:, 0].requires_grad, tem_joint_act[:, idx].requires_grad)
                q, _ = self.critic(self._build_inputs(batch, t=t), tem_joint_act)
                chosen_action_qvals.append(q.view(batch.batch_size, -1, 1))
                tem_joint_act.retain_grad()
        ...
        pg_loss.backward()
        # print("After grad:", tem_joint_act.grad[0].shape, tem_joint_act.grad[0])
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("loss_step/q_taken_mean", (q_taken * mask).sum().item() / mask_elems, t_env)
