...
class ManyAgentSwimmerEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, **kwargs):
        ...
        asset_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'assets', "manyagent", "swimmer",
                                                  'swimmer_{}_agents_each_{}_segments_{}.xml'.format(n_agents,
                                                                                                                 n_segs_per_agents, env_id))
        print("Auto-Generating {}-index Manyagent Swimmer asset with {} segments at {}.".format(env_id, n_segs, asset_path))
        self._generate_asset(n_segs=n_segs, asset_path=asset_path)
        # print("asset_path:", asset_path)
        # model = mujoco_py.load_model_from_path(asset_path)
        # print(mujoco_py.MjSim(model).data.qpos)
