...
class Particle(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        self.scenario.seed(rs=self.rs)   # set random state for scenario to generate uniform distribution
        ...
        if self.args.benchmark:
            ...
            if not self.args.partial_obs:
                ...
                self.env = OpenAIMultiAgentEnv(self.world,
                                               ...
                                               observable_agents=self.scenario.get_observable_agents)

        self.glob_args = kwargs.get("args")
        
        if self.seed:
            # print("--------Set env seed:", self.seed, "----numpy seed:", np.random.get_state()[1][0])
            self.env.seed(self.seed)
            # print("--------After set env seed:", self.seed, "----numpy seed:", np.random.get_state()[1][0])
        else:
            self.env.seed(0)  # set fixed seed for env
    ...
    def step(self, actions):
        ...
        # info_n['observable_agents'] = self.get_observable_agents()
    ...
    def get_observable_agents(self):
        """"return nearby agents within observation radius"""
        return [self.env._get_observable_agents(agent) for agent in self.world.policy_agents]
