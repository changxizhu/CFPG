...
class Scenario(BaseScenario):
    ...
    def seed(self, rs=None):
        # using rng for uniform otherwise it will change random seeds
        if rs:
            print("-------------------Scenario random state-------------------")
            self.rs = rs
        else:
            self.rs = np.random.RandomState(0)  # use default seed 0
    ...
    def prey_policy(self, agent, world):
        ...
        length = np.sqrt(self.rs.uniform(0, 1, n))
        angle = np.pi * self.rs.uniform(0, 2, n)
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            agent.state.p_pos = self.rs.uniform(-1, +1, world.dim_p)
            agent.state.p_vel = np.zeros(world.dim_p)
            agent.state.c = np.zeros(world.dim_c)
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                landmark.state.p_pos = self.rs.uniform(-0.9, +0.9, world.dim_p)
    ...
    def adversary_reward(self, agent, world):
        ...
        if agent.collide:
            """
            at least two agents collide with the agent then adversaries will get rewards
            1 agent: 1 reward
            2 and more agents: 10 rewards
            """
            for ag in agents:
                collide_sum = 0
                for adv in adversaries:                    
                ...
                if collide_sum == 1:
                    rew += 1
                if collide_sum > 1:
                    rew += 10  # if it's catch by multiple predators then use 10 for each/all
        return rew
    ...
    def get_observable_agents(self, agent, world):
        observable_list = []
        adversaries = self.adversaries(world)
        for other in adversaries:
            ...
            if agent.view_radius >= 0 and dist <= agent.view_radius:
                ...
                observable_list.append(0)
        
        # return observable agents
