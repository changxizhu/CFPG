...
# This multi-agent controller shares parameters between agents, and also use messages among agents
class CQMixMAC(BasicMAC):
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False, past_actions=None, critic=None,
        ...
        if self.args.agent in ["naf", "mlp", "rnn"]:
            # print("select actions:", bs, self.hidden_states.shape, self.hidden_states[bs].shape)
    ...
    def forward(self, ep_batch, t, actions=None, hidden_states=None, select_actions=False, test_mode=False, bs=slice(None)):
    ...
    def _build_inputs(self, batch, t, target_mac=False, last_target_action=None):
        ...
        bs = batch.batch_size
        # print("Batch size:", bs, "Obs shape:", batch["obs"].shape, t)
        inputs = []
        ...
        # print("actions after in buffer:", batch["actions"][:, t - 1].shape, batch["actions"][:, t - 1])
